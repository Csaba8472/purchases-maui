...
+ (void)presentCodeRedemptionSheet SWIFT_AVAILABILITY(watchos,unavailable) SWIFT_AVAILABILITY(macos,unavailable) SWIFT_AVAILABILITY(tvos,unavailable) SWIFT_AVAILABILITY(ios,introduced=14.0);
...
+ (void)addAttributionData:(NSDictionary<NSString *, id> * _Nonnull)data network:(NSInteger)network networkUserId:(NSString * _Nonnull)networkUserId SWIFT_DEPRECATED_MSG("Use the set<NetworkId> functions instead");
@end
